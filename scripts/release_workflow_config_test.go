...
func TestMakefileBenchGateUsesMergeBaseForRelatedNonAncestorRequestedBase(t *testing.T) {
	t.Parallel()
	...
	benchVars["MEMORY_BENCH_BASE"] = immutableBase
	benchVars["MEMORY_BENCH_PACKAGES"] = "./benchpkg"
	output, exitCode := runMakeTargetInDirExpectExitCode(t, repo, "bench-gate", benchVars, 0)
	if exitCode != 0 {
		t.Fatalf("bench-gate exit code = %d, want 0", exitCode)
	...
	for _, want := range []string{
		"Memory benchmark base ref '" + immutableBase + "' is related to HEAD but is not an ancestor; using merge base " + forkPoint + ".",
		"Running memory benchmark delta against " + forkPoint + " (requested " + immutableBase + ").",
