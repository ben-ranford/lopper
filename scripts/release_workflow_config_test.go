...
func TestMakefileBenchGatePreservesInvalidExitCodes(t *testing.T) {
	...
	for _, want := range []string{
		...
		`requested base ref '$$base_ref' is related to HEAD but is not an ancestor of HEAD`,
...
func TestMakefileBenchGateFailsClosedForRelatedNonAncestorRequestedBase(t *testing.T) {
	...
	output, _ := runMakeTargetInDirExpectExitCode(t, repo, "bench-gate", vars, 2)
	if !strings.Contains(output, "related to HEAD but is not an ancestor; failing closed") {
