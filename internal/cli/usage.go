...
const usage = `Usage:
  lopper [--version] [tui]
  lopper tui [--repo PATH] [--config PATH] [--language auto|all|js-ts|python|cpp|jvm|kotlin-android|go|php|ruby|rust|dotnet|elixir|swift|dart|powershell] [--top N] [--filter TEXT] [--sort name|waste] [--page-size N] [--ui-preference stave|legacy|ask] [--snapshot PATH] [--baseline PATH] [--baseline-store DIR] [--baseline-key KEY]
...
Options:
  ...
  --config PATH              Config file path for analyse, pr-review, and tui (relative to repo; default: .lopper.yml/.lopper.yaml/lopper.json)
  ...
  --ui-preference CHOICE    Remember stave or legacy, or ask again (interactive TUI only)
