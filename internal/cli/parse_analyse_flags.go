...
type analyseFlagValues struct {
	...
	thresholdReachableVulnPriority *string
...
func newAnalyseFlagSet(req app.Request) (*flag.FlagSet, analyseFlagValues) {
	...
	values := analyseFlagValues{
		...
		thresholdReachableVulnPriority: fs.String("threshold-reachable-vuln-priority", req.Analyse.Thresholds.ReachableVulnerabilityPriority, "fail when reachable vulnerability priority meets threshold (off, low, medium, high, critical)"),
