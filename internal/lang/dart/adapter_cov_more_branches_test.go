...
func TestDartAdditionalBranchCoverage(t *testing.T) {
	t.Run("detect and manifest loader errors", func(t *testing.T) {
	...
	t.Run("dependency metadata helpers", func(t *testing.T) {
	...
	t.Run("scan helpers and warnings", func(t *testing.T) {
		...
		if err := scanDartSourceFile(root, outsidePath, map[string]dependencyInfo{}, &scanResult{}); err == nil {
		...
		if err := scanPackageRoot(context.Background(), root, packageManifest{Root: filepath.Join(root, "missing"), Dependencies: map[string]dependencyInfo{}}, map[string]struct{}{}, map[string]struct{}{}, new(int), &scanResult{}); err == nil {
	...
	t.Run("import parsing and dependency selection", func(t *testing.T) {
