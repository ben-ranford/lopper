...
func TestRustAdapterCoverageHelpers(t *testing.T) {
	...
	name, local := normalizeUseSymbolNames(usePathEntry{Path: "serde::de::*", Wildcard: true}, "serde::de")
	if name != "*" || local != "de" {
	...
	if path, prefix, wildcard := normalizeUseWildcard("*", "serde::de"); !wildcard || path != "serde::de" || prefix != "" {
	...
	if dep := resolveDependency("::", "", map[string]dependencyInfo{}, scan); dep != "" {
