...
func TestDotNetAdditionalBranchCoverage(t *testing.T) {
	...
	t.Run("repo and scanner helper guards", func(t *testing.T) {
		...
		if err := collector.walk(repoEntryPath, objEntry, nil); !errors.Is(err, filepath.SkipDir) {
	...
	t.Run("unresolved imports and helper branches", func(t *testing.T) {
		...
		if binding, handled := parseCSharpImportLine([]byte("using Mystery.Component;"), "Program.cs", 1, 1, mapper, meta); !handled || binding == nil || binding.Dependency != "mystery.component" {
