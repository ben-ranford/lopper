...
const regressionProofProgramSource = "Program.cs"

func TestDotNetDiscoverScanInputsDoesNotRetainSourceDocuments(t *testing.T) {
	repo := t.TempDir()
	testutil.MustWriteFile(t, filepath.Join(repo, regressionProofProgramSource), "using Newtonsoft.Json;\n")
	...
	if len(scan.Files) != 1 || scan.Files[0].Path != regressionProofProgramSource {
