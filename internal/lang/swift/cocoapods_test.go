...
func TestSwiftAdapterCocoaPodsModuleMappings(t *testing.T) {
	...
	t.Run("subspec concatenation maps to module import", func(t *testing.T) {
		dependency := swiftFixturePodDependency{name: "Firebase/Analytics", version: "10.20.0"}
...
func TestSwiftCocoaPodsHelperFunctions(t *testing.T) {
	t.Run("catalog state active handles nil and present values", func(t *testing.T) {
	...
	t.Run("pod source helpers cover fallback sources", func(t *testing.T) {
		doc := podLockDocument{
			...
			ExternalSources: map[string]map[string]any{
				...
				"ArchivePod": {
					":http": "https://example.com/archive.zip",
				},
			},
		}
		...
		if got := extractPodSource(map[string]any{":http": "https://example.com/archive.zip"}); got != "https://example.com/archive.zip" {
			t.Fatalf("unexpected extracted http source: %q", got)
		}
		if got := podBaseName(" Firebase/Analytics "); got != "Firebase" {
			t.Fatalf("unexpected pod base name: %q", got)
		}
		if got := podSourceCandidates("Firebase/Analytics"); !slices.Equal(got, []string{"Firebase/Analytics", "Firebase"}) {
		...
		if got := lookupPodSource(doc.ExternalSources, "Firebase/Analytics"); got != "../Firebase" {
			t.Fatalf("expected base pod source lookup, got %q", got)
		}
		if got := lookupPodSource(nil, "Firebase/Analytics"); got != "" {
			t.Fatalf("expected empty lookup for nil source map, got %q", got)
		}
		if got := podLockSource(doc, "PrivatePod"); got != "https://example.com/private.git" {
			t.Fatalf("expected checkout options source, got %q", got)
		}
		if got := podLockSource(doc, "ArchivePod"); got != "https://example.com/archive.zip" {
			t.Fatalf("expected external source fallback, got %q", got)
		}
	})

	t.Run("pod parsing helpers cover variants and dedupe", func(t *testing.T) {
...
`))
	...
	t.Run("pod module helpers cover sdk aliases", func(t *testing.T) {
	...
	t.Run("load pod lock data captures external source metadata", func(t *testing.T) {
		repo := t.TempDir()
...
`)
		...
		if !meta.ResolvedViaCocoaPods || meta.Source != "https://example.com/private.git" {
			t.Fatalf("expected CocoaPods resolved metadata with git source, got %#v", meta)
		}
	})
}

func TestSwiftCocoaPodsLoaderBranches(t *testing.T) {
	t.Run("manifest loader handles errors warnings and invalid names", func(t *testing.T) {
		repo := t.TempDir()
	...
	t.Run("lock loader handles errors warnings and invalid names", func(t *testing.T) {
		repo := t.TempDir()
	...
	t.Run("parsers and warning helpers cover malformed inputs", func(t *testing.T) {
	...
	t.Run("catalog build surfaces cocoapods loader failures", func(t *testing.T) {
		repo := t.TempDir()
