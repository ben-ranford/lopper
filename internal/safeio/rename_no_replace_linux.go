...
func renameNoReplaceInRoot(root *osRoot, oldName, newName string) (returnErr error) {
	...
	if err := conn.Control(func(fd uintptr) {
		_, _, errno = syscall.Syscall6(sysRenameat2, fd, uintptr(unsafe.Pointer(oldPtr)), fd, uintptr(unsafe.Pointer(newPtr)), renameNoReplace, 0)
	}); err != nil {
		return err
	}
	if errno != 0 {
		return &os.LinkError{Op: "renameat2", Old: oldName, New: newName, Err: errno}
	}
	return nil
}
