...
func TestStaveTerminalDeepCoverageUpdateBranches(t *testing.T) {
	t.Run("command mode enter dispatches command", checkCommandModeEnterDispatchesCommand)

	t.Run("selection enter dispatches dependency aware open", checkSelectionEnterDispatchesDependencyAwareOpen)

	t.Run("refresh key dispatches refresh action", checkRefreshKeyDispatchesRefreshAction)

	t.Run("post-key quit short-circuits", checkPostKeyQuitShortCircuits)

	t.Run("paste failure is reported", checkPasteFailureIsReported)
}

func TestStaveTerminalDeepCoverageKeyPasteDispatchBranches(t *testing.T) {
	t.Run("key guard and success", checkKeyGuardAndSuccess)

	t.Run("multi rune and unsupported special key", checkMultiRuneAndUnsupportedSpecialKey)

	t.Run("paste guards, success and failure", checkPasteGuardsSuccessAndFailure)

	t.Run("dispatch text, validation and action branches", checkDispatchTextValidationAndActionBranches)
}

func TestStaveTerminalDeepCoverageViewResizeSessionReportActionBranches(t *testing.T) {
	t.Run("cancel inflight publishes correlated cancellation outcome", checkCancelInflightPublishesCorrelatedCancellationOutcome)

	t.Run("view error paths and fallbacks", checkViewErrorPathsAndFallbacks)

	t.Run("sessionSnapshot, resize and reportError", checkSessionsnapshotResizeAndReporterror)

	t.Run("dispatchAction event validation and runStaveTerminal exits", checkDispatchactionEventValidationAndRunstaveterminalExits)
}
...
func checkCommandModeEnterDispatchesCommand(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
...
func checkSelectionEnterDispatchesDependencyAwareOpen(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
		ctx:      context.Background(),
		prepared: struct{}{},
...
func checkRefreshKeyDispatchesRefreshAction(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
		ctx:      context.Background(),
		prepared: struct{}{},
...
func checkPostKeyQuitShortCircuits(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
		ctx:      context.Background(),
		prepared: struct{}{},
...
func checkPasteFailureIsReported(t *testing.T) {
	t.Helper()
...
func checkKeyGuardAndSuccess(t *testing.T) {
	t.Helper()
...
func checkMultiRuneAndUnsupportedSpecialKey(t *testing.T) {
	t.Helper()
	var count int
	b := &staveTerminal{
		ctx:      context.Background(),
...
func checkPasteGuardsSuccessAndFailure(t *testing.T) {
	t.Helper()
...
func checkDispatchTextValidationAndActionBranches(t *testing.T) {
	t.Helper()
	...
	b.inflight = false
	if b.beginCommand("plain text\x1b") == nil {
...
func checkCancelInflightPublishesCorrelatedCancellationOutcome(t *testing.T) {
	t.Helper()
...
func checkViewErrorPathsAndFallbacks(t *testing.T) {
	t.Helper()
...
func checkSessionsnapshotResizeAndReporterror(t *testing.T) {
	t.Helper()
...
func checkDispatchactionEventValidationAndRunstaveterminalExits(t *testing.T) {
	t.Helper()
	var published []event.Kind
	b := &staveTerminal{
		ctx:      context.Background(),
	...
	if b.beginAction(action.ID("bad\x1b"), map[string]any{"value": "x"}, false) == nil {
