...
func TestActionRegistryAndProgramHandlerBranches(t *testing.T) {
	...
	requireRegistryOpenHandler(t, prepared)

	requireRegistryBaselineHandlers(t, prepared, runner, summary, opts, store)

	requireRegistryCodemodHandler(t, prepared, runner)

	requireRegistryUnavailableHandlers(t, opts, view)
...
func TestActionRegistryRegistrationAndModelParityBranches(t *testing.T) {
	requireActionRegistrationGuards(t)
	...
	model = requireParityModelValueSemantics(t, model)
	model = requireParityCommandKeys(t, model)
	model = requireParityNavigationKeys(t, model)
	requireParityPageKeys(t, model)
}

func TestStaveParityProjectionComparisonAndKnownGapBranches(t *testing.T) {
	requireCoverageTreeProjection(t)
	...
	requireKnownGapComparisonRules(t)
...
func requireRegistryOpenHandler(t *testing.T, prepared *stave.Prepared[staveSummaryModel]) {
	t.Helper()
	...
	if _, err := completeLopperAction(context.Background(), t, prepared, action.ID(staveActionOpen), map[string]any{"dependency": "go:alpha"}, staveTestActionCall{sessionID: "lopper-preview", callID: "open", confirm: false}); err != nil {
...
func requireRegistryBaselineHandlers(t *testing.T, prepared *stave.Prepared[staveSummaryModel], runner *deepCoverageActionRunner, summary *Summary, opts Options, store string) {
	t.Helper()
	...
	if _, err := completeLopperAction(context.Background(), t, comparePrepared, action.ID(staveActionCompareBaseline), map[string]any{"file": runner.savePath}, staveTestActionCall{sessionID: "lopper-preview", callID: "compare", confirm: false}); err != nil {
		t.Fatalf("compare baseline failed: %v", err)
	}
}

func requireRegistryCodemodHandler(t *testing.T, prepared *stave.Prepared[staveSummaryModel], runner *deepCoverageActionRunner) {
	t.Helper()
...
func requireRegistryUnavailableHandlers(t *testing.T, opts Options, view summaryReportView) {
	t.Helper()
...
func requireActionRegistrationGuards(t *testing.T) {
	t.Helper()
...
func requireParityModelValueSemantics(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
...
func requireParityCommandKeys(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
...
func requireParityNavigationKeys(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
	model.interaction.quit = false
...
func requireParityPageKeys(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
...
func requireCoverageTreeProjection(t *testing.T) {
	t.Helper()
...
func requireKnownGapComparisonRules(t *testing.T) {
	t.Helper()
