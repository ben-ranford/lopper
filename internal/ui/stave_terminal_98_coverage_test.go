...
func TestStaveTerminalUpdateCoversDispatchQuitResizeAndKeyBranches(t *testing.T) {
	t.Run("dispatch error from command mode update", checkStaveTerminalDispatchErrorFromCommandModeUpdate)

	t.Run("selected action dispatch error and update quit", checkStaveTerminalSelectedActionDispatchErrorAndUpdateQuit)

	t.Run("resize and key fallback failures", checkStaveTerminalResizeAndKeyFallbackFailures)
}

func checkStaveTerminalDispatchErrorFromCommandModeUpdate(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
...
func checkStaveTerminalSelectedActionDispatchErrorAndUpdateQuit(t *testing.T) {
	t.Helper()
...
func checkStaveTerminalResizeAndKeyFallbackFailures(t *testing.T) {
	t.Helper()
...
func TestStaveTerminalTextPasteSnapshotAndDispatchActionBranches(t *testing.T) {
	t.Run("text event and paste parse/send failures", checkStaveTerminalTextEventAndPasteParseSendFailures)

	t.Run("dispatch and dispatchAction error branches", checkStaveTerminalDispatchAndDispatchactionErrorBranches)

	t.Run("session snapshot branches", checkStaveTerminalSessionSnapshotBranches)
}

func checkStaveTerminalTextEventAndPasteParseSendFailures(t *testing.T) {
	t.Helper()
...
func checkStaveTerminalDispatchAndDispatchactionErrorBranches(t *testing.T) {
	t.Helper()
	b := &staveTerminal{
		ctx:      context.Background(),
		prepared: struct{}{},
	...
	if b.beginCommand("bad\x1b") == nil {
		t.Fatal("invalid text dispatch was accepted")
	}
	b.inflight = false
	if b.beginAction(action.ID(""), nil, false) == nil {
...
func checkStaveTerminalSessionSnapshotBranches(t *testing.T) {
	t.Helper()
