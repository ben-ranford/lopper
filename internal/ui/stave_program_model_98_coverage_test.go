...
func TestStaveProgramConstructorViewAndRegistrySchemas(t *testing.T) {
	...
	requireProgramModelHash(t, program)
	...
	requireProgramRegistrySchemas(t, program)
}
...
func TestStaveProgramHandlersAndReducerBranches(t *testing.T) {
	...
	requireProgramRefreshAndOpen(t, prepared, program, state)

	requireProgramUnavailableHandlers(t, opts, view, state)

	requireProgramCodemodConfirmation(t, prepared, runner)

	requireProgramBaselineRoundTrip(t, prepared, runner, store)

	requireProgramBaselineErrors(t, runner, store)
...
func TestStaveModelHashAndEventBranches(t *testing.T) {
	...
	model = requireModelEffectCompletion(t, model)
	model = requireModelCommandEvents(t, model)
	requireModelRefreshEvents(t, model)
}
...
func requireProgramModelHash(t *testing.T, program stave.Program[staveSummaryModel]) {
	t.Helper()
...
type programActionSchemaExpectation struct {
	title   string
...
func requireProgramActionSchema(t *testing.T, program stave.Program[staveSummaryModel], id action.ID, want programActionSchemaExpectation) {
	t.Helper()
	...
	if want.confirm && (!def.Confirmation.Required || !def.Confirmation.SingleUse) {
...
func requireProgramRegistrySchemas(t *testing.T, program stave.Program[staveSummaryModel]) {
	t.Helper()
	...
	cases := map[action.ID]programActionSchemaExpectation{
	...
	for id, want := range cases {
		requireProgramActionSchema(t, program, id, want)
	}
}

func requireProgramRefreshAndOpen(t *testing.T, prepared *stave.Prepared[staveSummaryModel], program stave.Program[staveSummaryModel], state summaryState) {
	t.Helper()
	ctx := context.Background()
	if _, err := completeLopperAction(ctx, t, prepared, action.ID(staveActionRefresh), map[string]any{}, staveTestActionCall{sessionID: "coverage", callID: "refresh", confirm: false}); err != nil {
	...
	if _, err := completeLopperAction(ctx, t, prepared, action.ID(staveActionOpen), map[string]any{"dependency": "go:alpha"}, staveTestActionCall{sessionID: "coverage", callID: "open", confirm: false}); err != nil {
...
func requireProgramUnavailableHandlers(t *testing.T, opts Options, view summaryReportView, state summaryState) {
	t.Helper()
	ctx := context.Background()
...
func requireProgramCodemodConfirmation(t *testing.T, prepared *stave.Prepared[staveSummaryModel], runner *coverageProgramRunner) {
	t.Helper()
	ctx := context.Background()
	...
	if _, err := completeLopperAction(ctx, t, prepared, action.ID(staveActionApplyCodemod), map[string]any{"dependency": "go:alpha", "confirm": true, "allowDirty": true}, staveTestActionCall{sessionID: "coverage", callID: "codemod", confirm: true}); err != nil {
...
func requireProgramBaselineRoundTrip(t *testing.T, prepared *stave.Prepared[staveSummaryModel], runner *coverageProgramRunner, store string) {
	t.Helper()
	ctx := context.Background()
	if _, err := completeLopperAction(ctx, t, prepared, action.ID(staveActionSaveBaseline), map[string]any{"label": "nightly", "store": store}, staveTestActionCall{sessionID: "coverage", callID: "save", confirm: false}); err != nil {
	...
	if _, err := completeLopperAction(ctx, t, comparePrepared, action.ID(staveActionCompareBaseline), map[string]any{"file": runner.savePath}, staveTestActionCall{sessionID: "coverage", callID: "compare", confirm: false}); err != nil {
		t.Fatalf("compare baseline success failed: %v", err)
	}
}

func requireProgramBaselineErrors(t *testing.T, runner *coverageProgramRunner, store string) {
	t.Helper()
	ctx := context.Background()
...
func requireModelEffectCompletion(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
	var err error
...
func requireModelCommandEvents(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
	var err error
...
func requireModelRefreshEvents(t *testing.T, model staveSummaryModel) staveSummaryModel {
	t.Helper()
	var err error
