...
func TestPythonManifestStringsExtractsOnlyStrings(t *testing.T) {
	testCases := []struct {
		name  string
		value any
		want  []string
	}{
		{name: "string slice", value: []string{"requests", "flask"}, want: []string{"requests", "flask"}},
		{name: "mixed slice", value: []any{"requests", 3, "flask"}, want: []string{"requests", "flask"}},
		{name: "unsupported type", value: map[string]any{"name": "requests"}, want: nil},
	}

	for _, tc := range testCases {
		got := pythonManifestStrings(tc.value)
		if len(got) != len(tc.want) {
			t.Fatalf("%s: pythonManifestStrings() = %#v, want %#v", tc.name, got, tc.want)
		}
		for i := range tc.want {
			if got[i] != tc.want[i] {
				t.Fatalf("%s: pythonManifestStrings() = %#v, want %#v", tc.name, got, tc.want)
...
func TestCollectPipfileManifestEvidenceReadsPackagesAndDevPackages(t *testing.T) {
	repoPath := t.TempDir()
	pipfilePath := filepath.Join(repoPath, pythonPipfileName)
	testutil.MustWriteFile(t, pipfilePath, `[packages]
requests = "==2.32.3"
...
`)
	...
	index := identityIndex{}
	warnings := newIdentityWarningCollector(repoPath)
	collectPipfileManifestEvidence(repoPath, pipfilePath, index, warnings)

	assertSingleIdentityEvidence(t, index, identityKey("python", "requests"), "2.32.3", pythonPipfileName)
	assertSingleIdentityEvidence(t, index, identityKey("python", "pytest"), "8.3.0", pythonPipfileName)
	if got := warnings.list(); len(got) != 0 {
		t.Fatalf("expected Pipfile evidence without warnings, got %#v", got)
	}
}
