...
func TestV180FeatureQualificationDefaults(t *testing.T) {
	...
	for _, channel := range []Channel{ChannelDev, ChannelRelease} {
		t.Run(string(channel), func(t *testing.T) {
			resolved, err := DefaultRegistry().Resolve(ResolveOptions{Channel: channel})
			if err != nil {
			...
			for _, want := range stable {
				...
				disabled, err := DefaultRegistry().Resolve(ResolveOptions{Channel: channel, Disable: []string{want.name}})
				if err != nil {
					t.Fatalf("disable %s in %s: %v", want.name, channel, err)
				}
