...
hooks-install:
	@set -eu; \
		...
		rollback_install() { if [ "$$local_config_existed" = true ] && [ -f "$$local_config_backup" ] && ! mv -f "$$local_config_backup" "$$common_dir/config"; then return 1; elif [ "$$local_config_existed" = false ] && { [ -e "$$common_dir/config" ] || [ -L "$$common_dir/config" ]; } && ! rm -f "$$common_dir/config"; then return 1; fi; if [ "$$worktree_config_existed" = true ] && [ -f "$$worktree_config_backup" ] && ! mv -f "$$worktree_config_backup" "$$current_git_dir/config.worktree"; then return 1; fi; if [ "$$managed_hook_existed" = true ] && [ -f "$$backup_hook" ]; then mv -f "$$backup_hook" "$$managed_hook" || return 1; elif [ "$$managed_hook_existed" = false ]; then rm -f "$$managed_hook" || return 1; if [ "$$managed_dir_existed" = false ]; then rmdir "$$managed_dir" 2>/dev/null || :; fi; fi; }; \
