...
hooks-install:
	@set -eu; \
		...
		cleanup_preflight_temps() { cleanup_preflight_git; if [ "$$install_started" -eq 1 ] && [ "$$install_completed" -eq 0 ]; then config_restored=0; if [ -n "$$local_path" ]; then if git config --local core.hooksPath "$$local_path" >/dev/null 2>&1; then config_restored=1; fi; else if git config --local --unset core.hooksPath >/dev/null 2>&1; then config_restored=1; fi; fi; if [ "$$config_restored" -eq 1 ] || [ "$$activation_written" -eq 0 ]; then [ "$$created_hook" -eq 0 ] || rm -f "$$managed_hook"; [ "$$created_dir" -eq 0 ] || rmdir "$$managed_dir" 2>/dev/null || :; else echo "Unable to restore core.hooksPath; retaining managed hook at $$managed_hook" >&2; fi; fi; rm -f "$$config_error" "$$value_file" "$$values_file" "$$error_file" "$$markers_file" "$$references_file" "$$tmp_hook"; }; trap 'cleanup_preflight_temps; exit 129' HUP; trap 'cleanup_preflight_temps; exit 130' INT; trap 'cleanup_preflight_temps; exit 143' TERM; trap 'cleanup_preflight_temps' EXIT; \
