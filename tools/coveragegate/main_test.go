...
func TestParseCoverageProfileAndWriteOutputs(t *testing.T) {
	...
	totalOut := filepath.Join(dir, "total.txt")
	packagesOut := filepath.Join(dir, "packages.txt")
	failuresOut := filepath.Join(dir, "failures.txt")
...
func TestRunSuccessAndFailurePaths(t *testing.T) {
	...
	exitCode := run(passArgs, &stdout, &stderr)
	if exitCode != 0 {
...
func TestRunErrorPaths(t *testing.T) {
	...
	if exitCode != 2 {
		t.Fatalf("missing profile exit code = %d, want 2", exitCode)
	}
	...
	stdout.Reset()
	stderr.Reset()
	exitCode = run([]string{"-coverprofile", "missing.out", "-min", "-1"}, &stdout, &stderr)
	if exitCode != 2 {
		t.Fatalf("negative threshold exit code = %d, want 2", exitCode)
	}
	if !strings.Contains(stderr.String(), "-min must be non-negative") {
		t.Fatalf("expected threshold error, got %q", stderr.String())
	}

	stdout.Reset()
	stderr.Reset()
	exitCode = run([]string{"-coverprofile", "missing.out", "-package-min", "-1"}, &stdout, &stderr)
	if exitCode != 2 {
		t.Fatalf("negative package threshold exit code = %d, want 2", exitCode)
	}
	if !strings.Contains(stderr.String(), "-package-min must be non-negative") {
		t.Fatalf("expected package threshold error, got %q", stderr.String())
	}

	stdout.Reset()
	stderr.Reset()
	exitCode = run([]string{"-coverprofile", "missing.out", "-min", "0"}, &stdout, &stderr)
	if exitCode != 2 {
		t.Fatalf("missing profile file exit code = %d, want 2", exitCode)
	}
	if !strings.Contains(stderr.String(), "parse coverage profile:") {
		t.Fatalf("expected parse profile error, got %q", stderr.String())
	}

	stdout.Reset()
	stderr.Reset()
	exitCode = run([]string{"-unknown-flag"}, &stdout, &stderr)
	if exitCode != 2 {
		t.Fatalf("flag parse exit code = %d, want 2", exitCode)
	}
...
func TestRunWriteErrorPaths(t *testing.T) {
	...
	t.Run("total coverage write failure", func(t *testing.T) {
		stdout := &failOnWriteWriter{failOn: 1, err: errors.New("stdout failed")}
		var stderr bytes.Buffer

		exitCode := run([]string{"-coverprofile", profilePath, "-min", "0"}, stdout, &stderr)
		if exitCode != 2 {
			t.Fatalf("exit code = %d, want 2", exitCode)
		}
		if !strings.Contains(stderr.String(), "stdout failed") {
			t.Fatalf("expected stderr to contain write error, got %q", stderr.String())
		}
	})

	t.Run("package minimum write failure", func(t *testing.T) {
		stdout := &failOnWriteWriter{failOn: 2, err: errors.New("package minimum failed")}
		var stderr bytes.Buffer

		exitCode := run([]string{"-coverprofile", profilePath, "-min", "0", "-package-min", "98"}, stdout, &stderr)
		if exitCode != 2 {
			t.Fatalf("exit code = %d, want 2", exitCode)
		}
		if !strings.Contains(stderr.String(), "package minimum failed") {
			t.Fatalf("expected stderr to contain write error, got %q", stderr.String())
		}
	})

	t.Run("total failure message write failure", func(t *testing.T) {
		stdout := &failOnWriteWriter{failOn: 2, err: errors.New("failure message failed")}
		var stderr bytes.Buffer

		exitCode := run([]string{"-coverprofile", profilePath, "-min", "101"}, stdout, &stderr)
		if exitCode != 2 {
			t.Fatalf("exit code = %d, want 2", exitCode)
		}
		if !strings.Contains(stderr.String(), "failure message failed") {
			t.Fatalf("expected stderr to contain write error, got %q", stderr.String())
		}
	})

	t.Run("package failure heading write failure", func(t *testing.T) {
		stdout := &failOnWriteWriter{failOn: 3, err: errors.New("package failure heading failed")}
		var stderr bytes.Buffer

		exitCode := run([]string{"-coverprofile", profilePath, "-min", "0", "-package-min", "101"}, stdout, &stderr)
		if exitCode != 2 {
			t.Fatalf("exit code = %d, want 2", exitCode)
		}
		if !strings.Contains(stderr.String(), "package failure heading failed") {
			t.Fatalf("expected stderr to contain write error, got %q", stderr.String())
		}
	})

	t.Run("package failure body write failure", func(t *testing.T) {
		stdout := &failOnWriteWriter{failOn: 4, err: errors.New("package failure body failed")}
		var stderr bytes.Buffer
		...
		if !strings.Contains(stderr.String(), "package failure body failed") {
			t.Fatalf("expected stderr to contain write error, got %q", stderr.String())
		}
	})
}
...
func TestRunPackageFailureAndWriteErrorPaths(t *testing.T) {
	...
	packageFailArgs := []string{
		"-coverprofile", packageFailProfile,
		"-min", "97",
		"-package-min", "98",
	}
	...
	writeErrorArgs := []string{
		...
		"-min", "0",
		"-package-min", "0",
		"-total-out", filepath.Join(blockingFile, "total.txt"),
	}
	exitCode = run(writeErrorArgs, &stdout, &stderr)
	if exitCode != 2 {
		t.Fatalf("write error exit code = %d, want 2", exitCode)
	}
...
func TestParseCoverageLineAndHelpers(t *testing.T) {
	t.Run("parse valid uncovered line", func(t *testing.T) {
	...
	t.Run("parse invalid cases", func(t *testing.T) {
		cases := []string{
			"not enough fields",
	...
	t.Run("packageForFile", func(t *testing.T) {
	...
	t.Run("evaluateCoverage no package gate", func(t *testing.T) {
		report := coverageReport{
	...
	t.Run("round and format helpers", func(t *testing.T) {
...
func TestWriteOutputsPropagatesPackagesAndFailuresErrors(t *testing.T) {
	...
	if err := writeOutputs("", filepath.Join(packagesBlocker, "packages.txt"), "", report, result); err == nil || !strings.Contains(err.Error(), "write package coverage") {
	...
	if err := writeOutputs("", "", filepath.Join(failuresBlocker, "failures.txt"), report, result); err == nil || !strings.Contains(err.Error(), "write package failures") {
...
func TestMainExecutesRun(t *testing.T) {
	...
	cmd := exec.Command(os.Args[0], "-test.run=TestMainExecutesRun", "--", "-coverprofile", profilePath, "-min", "50")
