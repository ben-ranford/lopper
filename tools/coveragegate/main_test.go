...
func TestRunSuccessAndFailurePaths(t *testing.T) {
	...
	stdout, stderr, exitCode := runWithBuffers([]string{
...
func TestRunWriteErrorPaths(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:        []string{"-coverprofile", profilePath, "-min", "0"},
			failOnWrite: 1,
			errText:     "stdout failed",
		},
		{
			name:        "package minimum write failure",
			args:        []string{"-coverprofile", profilePath, "-min", "0", "-package-min", "98"},
			failOnWrite: 2,
			errText:     "package minimum failed",
		},
		{
			name:        "total failure message write failure",
			args:        []string{"-coverprofile", profilePath, "-min", "101"},
			failOnWrite: 2,
			errText:     "failure message failed",
		},
		{
			name:        "package failure heading write failure",
			args:        []string{"-coverprofile", profilePath, "-min", "0", "-package-min", "101"},
			failOnWrite: 3,
