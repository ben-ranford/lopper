...
The enabled gate reads approved policy from the target merge-base. A PR may reduce family membership but cannot authorize new members, new exceptions, or new canonical helper claims. Bootstrapping and expanding policy must use the separate reviewed workflow from #1612. The baseline schema is version 1: families contain explicit `members` and a nullable `canonical_helper`; exceptions contain an exact finding SHA-256, `rationale`, `owner`, `review`, and ISO-date `expires`. Directory patterns, incomplete metadata, duplicate findings, expired exceptions, and stale exceptions fail. A canonical helper must be a family member. Policy files should receive the same protected review as gate configuration.

This implementation does not enable the new gate in `make dup-check` before a baseline is approved. #1612 must commit the reviewed initial baseline, pin the detector threshold and invocation in a protected workflow, require the resulting status, and demonstrate on the exact protected revision that a known new clone is blocked and valid/approved-exception cases pass. The legacy percentage invocation remains until that rollout. Keep #1610 and #1611 open until #1612 records that evidence.
