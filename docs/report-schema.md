...
SBOM formats (`cyclonedx-json`, `cyclonedx-xml`, `spdx-json`, `spdx-tv`) are
separate output documents and do not use `docs/report-schema.json`.
...
- `languageBreakdown`: aggregate totals by adapter language (`js-ts`, `python`, `cpp`, `jvm`, `go`, `php`, `rust`, `dotnet`, `dart`).
