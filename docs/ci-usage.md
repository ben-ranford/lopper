...
- `make dup-check`: fail when **new/changed Go lines** exceed duplication max percentage versus the intended target's merge base (defaults: `DUPLICATION_MAX=3`, `DUPLICATION_TOKEN_THRESHOLD=55`). `DUPLICATION_BASE` explicitly selects a ref/SHA; otherwise the runner uses CI's immutable `BASE_SHA`, then the PR target (`GITHUB_BASE_REF` / `BASE_REF`), then local `origin/main`. Missing or unrelated bases fail with fetch/history recovery instructions, never a `HEAD~1` fallback.
