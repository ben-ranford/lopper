...
Lopper brings dependency-surface analysis into VS Code with inline diagnostics and hover context across supported adapters, plus safe JS/TS quick fixes powered by the `lopper` CLI.
...
- `lopper.language = auto` is the default. It prefers the active or saved editor's adapter when it can infer one, then falls back to `lopper` CLI auto detection.
- `lopper.language = all` runs every matching adapter in the workspace and merges the results.
- You can pin any supported adapter directly: `cpp`, `dart`, `dotnet`, `elixir`, `go`, `js-ts`, `jvm`, `php`, `python`, `ruby`, `rust`, or `swift`.
