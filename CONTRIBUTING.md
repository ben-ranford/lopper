...
To enable pre-commit validation, run `make hooks-install`. It installs or refreshes the managed hook in the repository's shared Git metadata, including upgrading an older installation. Every commit checks staged whitespace and Go formatting, then runs the full `make ci` target on the current working tree; any failure blocks the commit. Install the CI prerequisites above before using the hook. This executes the checkout's Makefile and scripts, so install and commit only in a checkout you trust. Unstaged changes are included in CI even though the whitespace and Go formatting checks use staged content. The installed hook snapshot stays fixed between explicit installations. `make hooks-uninstall` removes only Lopper's managed `core.hooksPath` setting and retains the installed snapshot.
