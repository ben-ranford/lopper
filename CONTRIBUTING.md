...
The installed hook checks staged whitespace and reports files that need `gofmt`; it does not run repository scripts, `make`, or Go tests. Run `make ci` explicitly before publishing changes; CI continues to require the full gate. Use `make hooks-uninstall` to remove the managed hook. It leaves unrelated custom hook paths intact and may retain the shared snapshot when conditional includes or foreign references cannot be ruled out.
