...
Run `make hooks-install` only from a trusted checkout, after reviewing `.githooks/pre-commit` and the `hooks-install` target in `Makefile`. The installer copies that hook into the repository's absolute shared Git metadata directory (`lopper-hooks`) before configuring `core.hooksPath`, so later branch checkouts cannot replace the hook Git executes. It refuses to replace an unrelated custom hook path. When worktree-specific Git config is enabled, clear or migrate non-managed hook paths in linked worktrees before installing. It also refuses hook configuration files that include other config files; consolidate the hook-path setting directly in the local or worktree config first. To preserve rollback safety, it also refuses symlink-backed common or current-worktree config files; use a regular local config before installing.

The installed hook checks staged whitespace and reports files that need `gofmt`; it does not run repository scripts, `make`, or Go tests. It ignores empty, relative, and checkout-controlled `PATH` entries, so Go tooling must be available from an absolute directory outside the checkout. Run `make ci` explicitly before publishing changes; CI continues to require the full gate. Use `make hooks-uninstall` to remove the managed hook. It leaves unrelated custom hook paths intact and may retain the shared snapshot when conditional includes or foreign references cannot be ruled out.
