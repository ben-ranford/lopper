...
To enable pre-commit validation, run `make hooks-install`. It installs or refreshes the managed hook in the repository's shared Git metadata, including upgrading an older installation. Every commit checks staged whitespace and Go formatting, then runs the full `make ci` target in a temporary worktree containing the exact staged snapshot; any failure blocks the commit. Install the CI prerequisites above before using the hook. This executes the staged Makefile and scripts, so install and commit only in a checkout you trust. Unstaged edits are excluded, HEAD-based gates see the pending commit, and the temporary worktree is removed afterward. Your branch and index are not changed. The installed hook snapshot stays fixed between explicit installations. `make hooks-uninstall` removes only Lopper's managed `core.hooksPath` setting and retains the installed snapshot.
