...
To enable pre-commit validation, run `make hooks-install`. It installs or refreshes the managed hook in the repository's shared Git metadata, including upgrading an older installation. Every commit checks staged whitespace and Go formatting, then runs the full `make ci` target in a temporary worktree containing the exact staged snapshot; any failure blocks the commit. Install the CI prerequisites above before using the hook. This executes the staged Makefile and scripts, so install and commit only in a checkout you trust. Unstaged edits are excluded, HEAD-based gates see the staged snapshot as a new commit, and the temporary worktree is removed afterward. Your branch and index are not changed. The installed hook snapshot stays fixed between explicit installations. Installation is the trust decision for that snapshot. At runtime, the Bash hook inventories main and linked checkouts using system Git, rejects selected Git/gofmt/make paths (including every symlink hop) inside any checkout, and carries a PATH without checkout directories into staged CI. External Homebrew and version-manager installations remain supported. Inventory errors fail closed; stale foreign worktree registrations are excluded without pruning them. Hook-owned Git commands disable pagers, filesystem monitors, and repository hooks. These guarantees cover hook-selected tools, not sandboxing the staged Makefile or scripts. `make hooks-uninstall` removes only Lopper's managed `core.hooksPath` setting and retains the installed snapshot.
Staged whitespace validation requires Git 2.41 or newer, using native `--attr-source` with the tree produced by `git write-tree`. Unsupported Git fails closed rather than falling back to checkout attributes. Repository `.gitattributes` files, including nested overrides, come only from that staged tree; HEAD and unstaged working-tree attributes do not override it. Git's usual precedence remains: `info/attributes` overrides repository attributes, which override global and system attributes; `core.whitespace` supplies the default when no whitespace attribute overrides it. Local configuration and non-repository attributes remain trusted policy. The hook freezes a private index from the staged tree and reuses it for whitespace, formatting, and CI, preserving alternate-index and linked-worktree inputs without changing the caller's index or working files. Do not concurrently modify the committing index or policy configuration: the hook validates one captured tree, and cannot lock external changes through Git's eventual commit operation.

When amending, use `LOPPER_HOOK_AMEND=1 git commit --amend` so ancestry-based gates see the amended commit's original parents. Git does not tell pre-commit whether `--amend` was requested; without this explicit mode the snapshot models a new child of HEAD, so ancestry-based checks may differ from post-commit CI for a history rewrite.
...
- The stable release workflow stamps `firstStableRelease` after a flag first ships in a semver release; it records shipping history and does not change lifecycle or graduation eligibility.
